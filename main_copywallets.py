...
Railway deployment — runs 24/7 with Persistent Volume & Live Web Server
"""
...
DATA_DIR            = os.getenv('DATA_DIR', '') # Default to local if not set, but on Railway MUST be /data/
EXCEL_FILE          = os.path.join(DATA_DIR, os.getenv('EXCEL_FILE', 'copy_trade_pnl.xlsx'))
...
# GitHub config (Only used for emergency large-file archiving)
    ...
    class FileHandler(SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            if self.path == '/':
                ...
                html = f"""
                <html>
                    <body style="font-family: Arial; text-align: center; margin-top: 50px; background-color: #1a1a2e; color: white;">
                        <h1>📈 Polymarket Tracker Live</h1>
                        <p style="color: #a0a0a0; margin-bottom: 40px;">Tracker is running 24/7. Current Excel size: {size_mb:.2f} MB</p>
                        <a href='/download' style="padding: 15px 30px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 18px; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
            ...
            # Download Route
                ...
                try:
                    with open(EXCEL_FILE, 'rb') as f:
                        file_data = f.read()
                    self.send_response(200)
                    self.send_header('Content-Disposition', 'attachment; filename="copy_trade_pnl.xlsx"')
                    ...
                    self.end_headers()
                    self.wfile.write(f"File not ready or error: {e}".encode('utf-8'))
            else:
                self.send_response(404)
                self.end_headers()

    # Bind to 0.0.0.0 to expose externally on Railway
    port = int(os.getenv('PORT', 8080))
    server = TCPServer(("0.0.0.0", port), FileHandler)
    log(f"🌐 Live Download Server running on port {port}")
    server.serve_forever()

# ═══════════════════════════════════════════════════════════
# LOCAL PERSISTENT RECOVERY & ARCHIVE
# ═══════════════════════════════════════════════════════════
...
def save_state(paper_balance, open_positions, closed_trades):
    ...
    try:
        ...
        log(f'⚠️ Failed to save state to volume: {e}')
    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, 'r') as f:
                state = json.load(f)
            log(f"💾 Memory recovered from volume! Restored {len(state.get('open_positions',{}))} open, {len(state.get('closed_trades',[]))} closed trades.")
            return state
        except Exception as e:
            log(f"⚠️ Could not read local state file: {e}")
    else:
        log("ℹ️ No previous memory found on volume. Starting fresh.")
    return None

def upload_to_github(file_path, commit_msg="Archive Sync"):
    if not GITHUB_TOKEN or not GITHUB_REPO:
        return
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{os.path.basename(file_path)}"
    try:
        ...
        if res.status_code in [200, 201]:
            log(f"☁️ Successfully archived to GitHub!")
        else:
            log(f"❌ GitHub archive failed: {res.text}")
    except Exception as e:
...
def _row(ws, r, vals, bg='FFFFFF', pnl_cols=None):
    ...
    for i, v in enumerate(vals, 1):
        ...
        if i in pnl_cols and isinstance(v, (int, float)):
            c.font = Font(name='Arial', size=9, bold=True, color='1a6b2e' if v >= 0 else '8b0000')
        else:
...
def init_excel():
    if os.path.exists(EXCEL_FILE):
        return # Skip if already on persistent volume
...
def main():
    ...
    # 1. Start the live download server in the background
    threading.Thread(target=start_web_server, daemon=True).start()

    log('🚀 Polymarket Copy Trade Tracker starting...')
    
    # 2. Initialize empty Excel if none exists
    init_excel()

    seen_tx        = {}
    watch_list     = {}

    # 3. Attempt to recover memory state directly from the Railway Volume
    state = load_state()
    if state:
    ...
    # 4. Rebuild Excel based on memory
    ...
    while True:
        ...
        # 🔥 Size Monitor: Checks the file size every 10 minutes (600 seconds)
        if time.time() - last_size_check >= 600:
            last_size_check = time.time()
            if os.path.exists(EXCEL_FILE):
                size_mb = os.path.getsize(EXCEL_FILE) / (1024 * 1024)
                log(f"📊 10-Min Check | Excel is currently taking up {size_mb:.2f} MB of volume storage.")
                
                # Push to GitHub ONLY if approaching the 50MB API limit
                if size_mb >= 45.0 and not has_uploaded_archive:
                    log(f"⚠️ DANGER: Excel size is {size_mb:.2f} MB! Approaching GitHub's 50MB hard limit.")
                    log("triggering one-time Archive push to GitHub...")
                    upload_to_github(EXCEL_FILE, f"Archive Backup - Size Limit Reached ({size_mb:.2f} MB)")
                    has_uploaded_archive = True
