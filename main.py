...
def init_excel():
    ...
    wb.save(EXCEL_FILE)
    log(f'📊 Excel created: {EXCEL_FILE}')
...
def main():
    ...
    log(f'   Balance: ${PAPER_BALANCE:,.0f} | Bet: {BET_SIZE_PCT*100:.0f}% | Top {TOP_N}/cat | Refresh every {LEADERBOARD_REFRESH} cycles')
    ...
    log('📸 Snapshotting existing activity...')
    for wallet, info in watch_list.items():
        ...
        log(f'   ✓ {info["username"]:<22} | {len(seen_tx[wallet]):>3} cached')
        time.sleep(0.3)
    ...
    while True:
        ...
        if cycle % LEADERBOARD_REFRESH == 0:
            ...
            new  = set(watch_list.keys())
            added   = [watch_list[w]['username'] for w in new - prev]
            removed = [w[:10] for w in prev - new]
            locked  = [watch_list[w]['username'] for w in watch_list if watch_list[w].get('locked')]
            if added:   log(f'   + Added   : {", ".join(added[:8])}')
            if removed: log(f'   - Removed : {", ".join(removed[:8])}')
            if locked:  log(f'   🔒 Locked  : {", ".join(locked)}')
            log(f'   Now watching {len(watch_list)} wallets')
            log_wallet_refresh(watch_list)
